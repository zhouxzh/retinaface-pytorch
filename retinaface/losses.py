...
def intersect(box_a, box_b):
    ...
    max_xy = torch.min(box_a[:, 2:].unsqueeze(1).expand(count_a, count_b, 2), box_b[:, 2:].unsqueeze(0).expand(count_a, count_b, 2))
    min_xy = torch.max(box_a[:, :2].unsqueeze(1).expand(count_a, count_b, 2), box_b[:, :2].unsqueeze(0).expand(count_a, count_b, 2))
...
def match(threshold, truths, priors, variances, labels, landms, loc_t, conf_t, landm_t, idx):
    ...
    for index in range(best_prior_idx.size(0)):
    ...
    conf_t[idx] = conf
    landm_t[idx] = encode_landm(matches_landm, priors, variances)
...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, priors, targets):
        ...
        pos_landm_idx = pos_landm.unsqueeze(pos_landm.dim()).expand_as(landm_data)
        ...
        normalizer = max(num_pos.data.sum().float(), 1)
        loss_l /= normalizer
        loss_c /= normalizer

        num_pos_landm = pos_landm.long().sum(1, keepdim=True)
        landm_normalizer = max(num_pos_landm.data.sum().float(), 1)
        loss_landm /= landm_normalizer
...
def weights_init(net, init_type='normal', init_gain=0.02):
    def init_func(module):
        ...
        if hasattr(module, 'weight') and classname.find('Conv') != -1:
            if init_type == 'normal':
...
def get_lr_scheduler(lr_decay_type, lr, min_lr, total_iters, warmup_iters_ratio=0.1, warmup_lr_ratio=0.1, no_aug_iter_ratio=0.3, step_num=10):
    def yolox_warm_cos_lr(lr, min_lr, total_iters, warmup_total_iters, warmup_lr_start, no_aug_iter, iters):
        ...
        if iters >= total_iters - no_aug_iter:
            return min_lr
        return min_lr + 0.5 * (lr - min_lr) * (1.0 + math.cos(math.pi * (iters - warmup_total_iters) / (total_iters - warmup_total_iters - no_aug_iter)))
    ...
    if lr_decay_type == 'cos':
        ...
        return partial(yolox_warm_cos_lr, lr, min_lr, total_iters, warmup_total_iters, warmup_lr_start, no_aug_iter)
